...
`org.fmi-standard.fmi-ls-struct.map.CombiTable1Ds`::
for maps represented by a single parameter matrix variable in which both the domain and codomain values are stored together (as in link:https://doc.modelica.org/Modelica%204.0.0/Resources/helpDymola/Modelica_Blocks_Tables.html#Modelica.Blocks.Tables.CombiTable1Ds[Modelica.Blocks.Tables.CombiTable1Ds])
