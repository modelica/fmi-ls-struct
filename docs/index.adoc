...
Requirement for the matching: The number of domains must be the same and the names(+type+size) of all domains (i.e., `variableKind=org.fmi-standard.fmi-ls-struct.domain`) must match. If all domains match, connect all these domains. Connect all co-domains (`variableKind=org.fmi-standard.fmi-ls-struct.`) that match. 
...
*  org.fmi-standard.fmi-ls-struct.domainInput
